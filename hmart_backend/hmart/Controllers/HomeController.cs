    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            HomeVM homeVM = new HomeVM
            {
                ...
                .Include(x => x.ProImages)
                .Include(x => x.Category)
                .OrderByDescending(x => x.Rate).Take(8).ToList(),

                FeaturedProducts = _context.Products
                .Include(x => x.ProImages)
                .Include(x => x.Category)
                .Where(x => x.IsOnOffer).Take(8).ToList(),

                MainOffer = _context.Offers
                .Include(x => x.Product).ThenInclude(x=>x.ProImages)
                .FirstOrDefault(x => x.IsMain),

                Offers = _context.Offers.Where(x=>x.IsMain==false)
                .Include(x=>x.Product).Take(2).ToList(),
